...
//			$bidwhen_date = date("Ymd h:i:s A",$row["bidwhen"]);
			...
			$tplv .= "<div>"; 			
			if($won_bid == $bid) {
				$tplv .= "<div class='bidHistory lowUnique'><span color='#ffffff'><b>&pound;".$bid."</b></span></div>";	
			}elseif(in_array($bid,$unique)){
				$tplv .= "<div class='bidHistory unique'><span color='#ffffff'><b>&pound;".$bid."</b></span></div>";	
			}else{
				$tplv .= "<div class='bidHistory notUnique'><span color='#ffffff'>&pound;".$bid."</span></div>";	
			}			
