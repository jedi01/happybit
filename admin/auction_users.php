...
#  *** put a primary key on the first place 
    ...
    $sql = "SELECT users.id, users.nick, COUNT(bids.bidder) AS bid_count, 'View offers' AS link
            FROM HB_bids b
                INNER JOIN HB_users u ON bids.bidder=users.id
            WHERE bids.auction=$auction_id
            GROUP BY bids.bidder";

#  *** set needed options and create a new class instance 
  $debug_mode = false;        /* display SQL statements while processing */    
  $messaging = false;          /* display system messages on a screen */ 
  $unique_prefix = "au_";    /* prevent overlays - must be started with a letter */
...
##  ***      "barchart" : number format in SELECT SQL must be equal with number format in max_value
 $vm_colimns = array(
    "nick"     =>array("header"=>"Nick",    "type"=>"label", "align"=>"left", "width"=>"60%",        "wrap"=>"nowrap", "text_length"=>"-1", "tooltip"=>true,  "tooltip_type"=>"simple", "case"=>"normal", "summarize"=>"false", "sort_by"=>"", "visible"=>"true", "on_js_event"=>""),
    "bid_count"=>array("header"=>"Bid count",    "type"=>"label", "align"=>"center", "width"=>"20%",        "wrap"=>"nowrap", "text_length"=>"-1", "tooltip"=>true,  "tooltip_type"=>"simple", "case"=>"normal", "summarize"=>"false", "sort_by"=>"", "visible"=>"true", "on_js_event"=>""),
    "link"=>array("header"=>"&nbsp;",    "type"=>"link",  "align"=>"center", "width"=>"20%", "wrap"=>"nowrap", "text_length"=>"-1", "tooltip"=>false, "tooltip_type"=>"simple", "case"=>"normal", "summarize"=>"false", "sort_by"=>"", "visible"=>"true", "on_js_event"=>"", "field_key"=>"id", "field_data"=>"link", "rel"=>"", "title"=>"", "target"=>"", "href"=>"auction_users_bids.php?aid=$auction_id&uid={0}"),
...
<?
    echo "<center><a class='default_class_a' href='listallauctions.php'>Back</a></center>";
?>
</BODY>
</HTML>